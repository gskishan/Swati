...
class SalarySlipProjectWise(TransactionBase):
	def __init__(self, *args, **kwargs):
		super().__init__(*args, **kwargs)
		self.series = f"Project Wise Sal Slip/{self.employee}/.#####"
	...
	def set_time_sheet(self):
		if self.salary_slip_based_on_timesheet:
			...
			timesheets = (
				...
				.select(Timesheet.name, Timesheet.total_hours)  # Fixed: `Timesheet.star` -> `Timesheet.name`
				.where(
					(Timesheet.employee == self.employee)
					& (Timesheet.project == self.project)  # 🔥 Added Project Filter
					& (Timesheet.start_date.between(self.start_date, self.end_date))
					& ((Timesheet.status == "Submitted") | (Timesheet.status == "Billed"))
				)
			).run(as_dict=True)  # Fixed: `as_dict=1` -> `as_dict=True`
	...
	def check_sal_struct(self):
		...
		ssa_child = frappe.qb.DocType("Salary Structure Assigment")  # Child table (same as parent)

		query = (
			frappe.qb.from_(ssa_child)
			.join(ssa)
			.on(ssa_child.parent == ssa.name)  # Link child table to parent
			.join(ss)
			.on(ssa_child.salary_structure == ss.name)  # Link child table to Salary Structure
			.select(ssa_child.salary_structure)
			.where(
				(ssa.docstatus == 1)
				& (ss.docstatus == 1)
				& (ss.is_active == "Yes")
				& (ssa.employee == self.employee)
				& (ssa_child.project == self.project)  # 🔥 Get Salary Structure for specific Project
				& (
		...
		if st_name:
			self.salary_structure = st_name[0][0]  # Fetch Salary Structure from Child Table
			return self.salary_structure

		else:
			self.salary_structure = None
			frappe.msgprint(
				_("No active Salary Structure found for employee {0} for project {1} and given dates").format(
					self.employee, self.project
				),
	...
	def get_working_days_details(self, lwp=None, for_preview=0):
		...
		# Fetch attendance grouped by project
		attendance_records = frappe.db.sql(
			"""
			SELECT attendance_date, custom_project 
			FROM `tabAttendance` 
		...
		# Group attendance dates by project
		project_attendance = {}
		for record in attendance_records:
			project = record.custom_project
			date = getdate(record.attendance_date)
			project_attendance.setdefault(project, []).append(date)

		# Identify attendance clusters (continuous attendance periods)
		project_clusters = {}
		for project, dates in project_attendance.items():
			sorted_dates = sorted(dates)
			clusters = []
			cluster_start = cluster_end = sorted_dates[0]

			for i in range(1, len(sorted_dates)):
				if (sorted_dates[i] - cluster_end).days == 1:  # Continuous attendance
					cluster_end = sorted_dates[i]
				else:
					clusters.append((cluster_start, cluster_end))
					cluster_start = cluster_end = sorted_dates[i]

			clusters.append((cluster_start, cluster_end))
			project_clusters[project] = clusters

		# Fetch holidays within attendance range
		attendance_start = min(d for dates in project_attendance.values() for d in dates)
		attendance_end = max(d for dates in project_attendance.values() for d in dates)
		holidays = set(self.get_holidays_for_employee(attendance_start, attendance_end))

		# ✅ FIXED: Only allocate holidays to the CURRENT project of this salary slip
		project_holiday_count = {project: 0 for project in project_attendance}

		for holiday in holidays:
			assigned_project = None

			# Find the project whose attendance cluster is closest to the holiday
			for project, clusters in project_clusters.items():
				if project != self.project:  # ✅ Only count holidays for the CURRENT project
					continue
				for start, end in clusters:
					if start <= holiday <= end or (holiday - end).days == 1:
			...
			if assigned_project:
				project_holiday_count[assigned_project] += 1

		# ✅ FIXED: Compute working days **ONLY for self.project**
		attendance_days = len(project_attendance.get(self.project, []))
		holiday_days = project_holiday_count.get(self.project, 0)
		total_days = attendance_days + holiday_days

		# ✅ Only update for the current project
	...
	def get_employee_attendance(self, start_date, end_date):
		...
		attendance_details = (
			...
			.where(
				...
				& (attendance.employee == self.employee)
				& (attendance.custom_project == self.project)  # 🔥 Added Project Filter
				& (attendance.docstatus == 1)
	...
	def set_salary_structure_assignment(self):
		self._salary_structure_assignment = frappe.db.get_value(
			"Salary Structure Assignment Project Wise",
			{
				"employee": self.employee,
...
def get_date_clusters(dates):
    """Groups consecutive dates into clusters (attendance + linked holidays)."""
    if not dates:
        return []
    ...
    clusters.append((start, prev))  # Last cluster
    return clusters
